...
An Agent token occurrence may also set `max_rows` to wrap instead of truncating:
...
`max_rows` defaults to `1`, which keeps the single truncated row. A higher value (at most 16) lets that token continue onto extra rows, breaking on whitespace and hard-splitting a single word that is longer than the panel. Continuation rows are indented under the wrapped token, and the final permitted row is still elided with `…` when text remains. Entries grow by exactly the rows they use, so scrolling and click targets follow. `max_rows` is honored for Agent rows only.
