...
fn tab_width(ws: &crate::workspace::Workspace, tab_idx: usize, show_numbers: bool) -> u16 {
    display_width_u16(&tab_chrome_label(ws, tab_idx, show_numbers))
        .saturating_add(4)
        .max(MIN_TAB_WIDTH)
}
...
fn tab_chrome_label(
    ...
    // Prefix the switch index so the label lines up with the switch_tab 1..9
    // bindings. Auto-named tabs already render as the bare index.
    let named = ws.tabs.get(tab_idx).is_some_and(|tab| !tab.is_auto_named());
    let name = if show_numbers && named {
        format!("{number}:{display_name}")
    } else {
        display_name
    };
...
fn layout_tab_hit_areas(
    ...
    for (idx, rect) in rects.iter_mut().enumerate().skip(scroll) {
        ...
        let desired = tab_width(ws, idx, show_numbers);
...
fn centered_tab_scroll(ws: &crate::workspace::Workspace, area: Rect, show_numbers: bool) -> usize {
    ...
    for scroll in 0..=ws.active_tab {
        let rects = layout_tab_hit_areas(ws, area, scroll, show_numbers);
...
fn max_tab_scroll(ws: &crate::workspace::Workspace, area: Rect, show_numbers: bool) -> usize {
    (0..ws.tabs.len())
        .find(|&scroll| {
            layout_tab_hit_areas(ws, area, scroll, show_numbers)
                .last()
...
pub(crate) fn compute_tab_bar_view(
    ...
    if !mouse_chrome {
        let max_scroll = max_tab_scroll(ws, area, show_numbers);
        let scroll = if follow_active {
            centered_tab_scroll(ws, area, show_numbers).min(max_scroll)
        } else {
            current_scroll.min(max_scroll)
        };
        return TabBarView {
            scroll,
            tab_hit_areas: layout_tab_hit_areas(ws, area, scroll, show_numbers),
    ...
    let all_tabs = layout_tab_hit_areas(ws, all_tabs_area, 0, show_numbers);
    ...
    let max_scroll = max_tab_scroll(ws, tab_area, show_numbers);
    let scroll = if follow_active {
        centered_tab_scroll(ws, tab_area, show_numbers).min(max_scroll)
    } else {
        current_scroll.min(max_scroll)
    };
    let tab_hit_areas = layout_tab_hit_areas(ws, tab_area, scroll, show_numbers);
...
mod tests {
    ...
    #[test]
    fn tab_numbers_prefix_named_tabs_only() {
        let mut ws = Workspace::test_new("test");
        let named = ws.test_add_tab(Some("build"));
        ws.tabs[0].custom_name = None;

        assert_eq!(tab_chrome_label(&ws, 0, false), "1");
        assert_eq!(tab_chrome_label(&ws, 0, true), "1");
        assert_eq!(tab_chrome_label(&ws, named, false), "build");
        assert_eq!(tab_chrome_label(&ws, named, true), "2:build");
    }
    ...
    fn tab_number_prefix_counts_toward_tab_width() {
        ...
        assert_eq!(tab_width(&ws, 0, false), 12);
        assert_eq!(tab_width(&ws, 0, true), 14);
    }
